...
{'INPUT':'C:PATH_TO_DATA/streets.shp','EXTENT':'291035.789300000,297662.225800000,5035249.34
4900000,5039316.111100000 [EPSG:2950]','CLIP':True,'OUTPUT':'TEMPORARY_OUTPUT'})
```

- Paste this code snippet into a new, empty script. Give the OUTPUT value a filename with the .shp
extension (you could also format your code as seen below for readability).
- Replace PATH_TO_DATA with the actual path to your streets file.
- Save the script as clip_extent.py and Run the script.

This should produce the same output as you had before, except this time, you might notice the file is located
elsewhere on your computer. You might also notice it wasn't added to your map window. Why is this? What
variables and/or functions could we have included for this not to happen (see part 1…) ?
...
- We can also pinpoint in what cases italics will apply: next to the I symbol > Edit…
  - `"CLASSE" = '8'`
- Above, by going to the Edit… window for Value, we can even filter what text is being used for our
street labels:
...
You should now have a cartographically-optimized street network than you had before! As you can see,
...
- A [module from an online course](https://autogis-site.readthedocs.io/en/2019/lessons/L7/pyqgis.html) on automatic GIS workflows
- Another [overview](https://www.geodose.com/p/pyqgis.html) by a fairly active GIS blog
- A [video walkthrough](https://www.youtube.com/watch?v=h-mpUkwDdOQ) of QGIS expressions and variables by another very active contributor Nyall Dawson
